#!/bin/bash
# TabICLv2 pre-training -- REGRESSOR, Stage 1 (of 3).
# (The classifier is a separate checkpoint; see train_v2_clf_stage{1,2,3}.sh.)
#
# Recipe from "TabICLv2: A better, faster, scalable, and open tabular foundation
# model" (https://arxiv.org/abs/2602.11139), §4.1 / Appendix B.1. The regressor uses
...
# --num_quantiles 999; the trainer sets the model's max_classes to 0 and switches
# the target embeddings to linear layers); the graph prior keeps the target
# continuous (outlier-removed and standard-scaled inside the prior; ~25% of stage-1
# datasets are filtered as unpredictable); and bias-free LayerNorm
...
CKPT_DIR=/path/to/checkpoints/tabiclv2-reg/stage1
