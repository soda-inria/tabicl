...
MODULE_PATH = REPO_ROOT / "238_iepesudo_tabicl.py"


def load_module():
    module_name = "module_238_iepesudo_tabicl"
...
def test_run_entropy_two_pass_pseudo_appends_predicted_labels_and_writes_entropy(tmp_path):
    clf = FakePseudoClassifier(np.array([[0.01, 0.99], [0.55, 0.45]]))
    ...
    np.testing.assert_array_equal(y_pred, np.array([1, 0]))
