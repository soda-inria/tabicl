...
def test_run_correct_prediction_resplit_inference_refits_with_only_correct_samples():
    clf = FakePseudoClassifier(np.array([[0.99, 0.01], [0.01, 0.99]]))
    ...
    result = pesudo_tabicl_test.run_correct_prediction_resplit_inference(
    ...
    assert result["pseudo_rule"] == "correct_prediction_only"
...
def test_run_correct_prediction_resplit_inference_falls_back_when_no_correct_sample():
...
def test_run_correct_prediction_resplit_inference_ignores_high_confidence_wrong_samples():
    ...
    assert result["baseline_accuracy"] == 0.5
    assert len(clf.fit_labels) == 2
    assert 1 in clf.fit_labels[1]
    assert 0 not in clf.fit_labels[1]


def test_run_correct_prediction_resplit_inference_returns_baseline_when_all_correct():
    clf = FakePseudoClassifier(np.array([[0.01, 0.99], [0.99, 0.01]]))
    ...
    assert result["pseudo_added"] == len(y_test)
    assert result["pseudo_applied"] is False
    assert result["baseline_accuracy"] == 1.0
    assert len(clf.fit_labels) == 1
    np.testing.assert_array_equal(result["y_eval"], y_test)
    np.testing.assert_array_equal(result["y_pred"], y_test)


def test_write_results_adds_correct_only_stats_to_summary(tmp_path):
    results = [
        {
            "dataset": "alpha",
            "accuracy": 0.75,
    ...
    pesudo_tabicl_test.write_results(
        tmp_path,
        results,
    ...
    assert "Pseudo label rule: correct_prediction_only" in summary_text
    assert "Pseudo label CE alpha" not in summary_text
    assert "Datasets with pseudo update applied: 2" in summary_text


def test_build_arg_parser_drops_legacy_pseudo_label_ce_alpha():
    parser = pesudo_tabicl_test.build_arg_parser()
    args = parser.parse_args([])
    assert args.outdir == "tabiclv1.1_pesudo_test_correct_only"
    assert not hasattr(args, "pseudo_label_ce_alpha")


def test_main_rejects_legacy_pseudo_label_ce_alpha_argument(tmp_path):
    tmp_path.mkdir(exist_ok=True)

    with pytest.raises(SystemExit):
        pesudo_tabicl_test.main(
            [
                "--data-root",
                str(tmp_path),
