...
class TTTConfig:
    ...
    icl_blocks_from_end: Optional[tuple[int, ...]] = None
    train_icl_head: bool = False
...
def parse_icl_blocks_from_end(value: str) -> tuple[int, ...] | None:
    normalized = value.strip().lower()
    if normalized == "all":
        return None
    if not normalized:
        raise argparse.ArgumentTypeError("must be 'all' or a comma-separated list of positive integers")
    ...
    for raw_part in normalized.split(","):
        part = raw_part.strip()
        if not part:
            raise argparse.ArgumentTypeError("empty block id in comma-separated list")
        try:
            block_id = int(part)
        except ValueError as exc:
            raise argparse.ArgumentTypeError("block ids must be positive integers") from exc
        if block_id < 1:
            raise argparse.ArgumentTypeError("block ids must be >= 1")
...
def build_ttt_config(args: argparse.Namespace) -> TTTConfig:
    ...
    return TTTConfig(
        ...
        icl_blocks_from_end=args.ttt_icl_blocks_from_end,
        train_icl_head=bool(args.ttt_train_icl_head),
...
def _get_icl_transformer_blocks(model):
    try:
        return model.icl_predictor.tf_icl.blocks
    except AttributeError as exc:
        raise RuntimeError("Expected ICL blocks at model.icl_predictor.tf_icl.blocks") from exc


def _resolve_icl_block_indices_from_end(model, blocks_from_end: tuple[int, ...] | None) -> List[int]:
    icl_blocks = _get_icl_transformer_blocks(model)
    num_blocks = len(icl_blocks)
    if num_blocks < 1:
        raise RuntimeError("ICL predictor has no transformer blocks")

    if blocks_from_end is None:
        return list(range(num_blocks))

    selected_indices: list[int] = []
    seen: set[int] = set()
    for block_from_end in blocks_from_end:
        if block_from_end < 1:
            raise ValueError("--ttt-icl-blocks-from-end values must be >= 1")
        if block_from_end > num_blocks:
            raise ValueError(
                f"--ttt-icl-blocks-from-end value {block_from_end} exceeds "
                f"available ICL blocks ({num_blocks})"
            )
        block_idx = num_blocks - block_from_end
...
def _set_icl_head_requires_grad(icl_predictor, requires_grad: bool) -> None:
    for attr_name in ("y_encoder", "ln", "decoder"):
        module = getattr(icl_predictor, attr_name, None)
...
def _configure_ttt_trainable_params(classifier, config: TTTConfig):
    ...
    icl_blocks = _get_icl_transformer_blocks(model)
    for block_idx in _resolve_icl_block_indices_from_end(model, config.icl_blocks_from_end):
        _set_requires_grad(icl_blocks[block_idx], True)

    if config.train_icl_head:
        _set_icl_head_requires_grad(model.icl_predictor, True)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--out-dir", default="block_1b_result/ensmble_step10_block12")
    ...
    parser.add_argument(
        "--ttt-icl-blocks-from-end",
        type=parse_icl_blocks_from_end,
        default=(1,),
        help=(
            "ICL transformer blocks to train during TTT. Use 'all' for every block, "
            "or a comma-separated 1-based list counted from the end, e.g. '1,2,3'."
        ),
    )
    parser.add_argument(
        "--ttt-train-icl-head",
        type=parse_bool,
        default=True,
        help="Whether to train ICL non-block modules: y_encoder, ln, and decoder.",
    )
    parser.add_argument(
        "--ttt-save-ckpt",
        type=parse_bool,
