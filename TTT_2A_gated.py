...
class ResultRow:
    ...
    ttt_update_ran: bool = False
    ttt_gate_passed: bool = False
    ttt_applied: bool = False
    ttt_update_seconds: float = 0.0
    ttt_split_strategy: Optional[str] = None
    ttt_split_reason: Optional[str] = None
    gate_baseline_accuracy: Optional[float] = None
    gate_adapted_accuracy: Optional[float] = None
    gate_accuracy_delta: Optional[float] = None
    gate_baseline_loss: Optional[float] = None
    gate_adapted_loss: Optional[float] = None
    gate_loss_delta: Optional[float] = None
    ttt_gate_threshold: Optional[float] = None
...
class TTTConfig:
    ...
    b_fraction: float = 0.6
    c_fraction: float = 0.3
    d_fraction: float = 0.1
    gate_acc_threshold: float = 0.0
    random_state: int = 42
    ...
    *,
    ...
    if row.status == "ok":
        return (
            ...
            f"ttt_gate_passed={row.ttt_gate_passed} "
            f"gate_delta={format_optional_float(row.gate_accuracy_delta)} "
...
def make_feature_frame(values, *, kind: str, prefix: str):
    ensure_runtime_deps()
...
def make_target_array(values) -> Any:
    ensure_runtime_deps()
...
def build_ttt_config(args: argparse.Namespace) -> TTTConfig:
    fractions = (
        float(args.ttt_b_fraction),
        float(args.ttt_c_fraction),
        float(args.ttt_d_fraction),
    )
    if any(value <= 0.0 for value in fractions):
        raise ValueError("TTT B/C/D fractions must all be > 0")
    if abs(sum(fractions) - 1.0) > 1e-8:
        raise ValueError("--ttt-b-fraction + --ttt-c-fraction + --ttt-d-fraction must equal 1.0")
    ...
    return TTTConfig(
        ...
        b_fraction=float(args.ttt_b_fraction),
        c_fraction=float(args.ttt_c_fraction),
        d_fraction=float(args.ttt_d_fraction),
        gate_acc_threshold=float(args.ttt_gate_acc_threshold),
...
def _train_test_split_indices(
    ...
    try:
        left_idx, right_idx = train_test_split(
            ...
            shuffle=True,
            stratify=np.asarray(y_values),
        )
        return (
            np.asarray(left_idx, dtype=int),
            np.asarray(right_idx, dtype=int),
            "stratified",
            f"{stage_name}: stratified split",
        )
    except Exception as exc:
        left_idx, right_idx = train_test_split(
            indices,
        ...
        return (
            ...
            np.asarray(right_idx, dtype=int),
            "random",
            f"{stage_name}: stratified split unavailable: {type(exc).__name__}: {exc}",
        )


def _resolve_split_counts(total_count: int, config: TTTConfig) -> tuple[int, int, int]:
    raw_counts = np.asarray(
        [
            total_count * config.b_fraction,
            total_count * config.c_fraction,
            total_count * config.d_fraction,
        ],
        dtype=np.float64,
    )
...
def split_ttt_gated_holdout(y, config: TTTConfig) -> TTTSplit:
    ensure_runtime_deps()
    ...
    if len(indices) < 3:
        return TTTSplit(
            ...
            d_indices=empty,
            strategy="none",
            reason="Need at least three samples for B/C/D split.",
        )

    y_array = np.asarray(y)
    b_count, c_count, d_count = _resolve_split_counts(len(indices), config)

    b_idx, cd_idx, stage1_strategy, stage1_reason = _train_test_split_indices(
        indices,
        y_array,
        train_size=b_count,
    ...
    if c_count <= 0 or d_count <= 0 or len(cd_idx) < 2:
        return TTTSplit(
            b_indices=b_idx,
            c_indices=cd_idx,
            d_indices=empty,
            strategy=stage1_strategy,
            reason=f"{stage1_reason} | Remaining CD split needs at least two samples.",
        )

    c_idx, d_idx, stage2_strategy, stage2_reason = _train_test_split_indices(
        cd_idx,
        y_array[cd_idx],
        train_size=c_count,
    ...
    strategy = "stratified" if stage1_strategy == "stratified" and stage2_strategy == "stratified" else "random"
    return TTTSplit(
        b_indices=b_idx,
        c_indices=c_idx,
        d_indices=d_idx,
...
def _label_subset_reason(y_b, y_c, y_d) -> Optional[str]:
    y_b_values = set(pd.Series(np.asarray(y_b)).astype(object).tolist())
    reasons = []

    missing_in_c = sorted(str(value) for value in (set(pd.Series(np.asarray(y_c)).astype(object).tolist()) - y_b_values))
    if missing_in_c:
        reasons.append("C labels absent from B context: " + ",".join(missing_in_c))

    missing_in_d = sorted(str(value) for value in (set(pd.Series(np.asarray(y_d)).astype(object).tolist()) - y_b_values))
    if missing_in_d:
        reasons.append("D labels absent from B context: " + ",".join(missing_in_d))
...
def evaluate_holdout_metrics(classifier, X_holdout, y_holdout) -> GateEvalResult:
    ensure_runtime_deps()

    y_true = np.asarray(y_holdout)
    if y_true.shape[0] == 0:
        raise ValueError("Holdout split must contain at least one sample")

    proba = np.asarray(classifier.predict_proba(X_holdout), dtype=np.float64)
    if proba.ndim != 2 or proba.shape[0] != y_true.shape[0]:
        raise ValueError(f"Unexpected predict_proba shape: {proba.shape}")

    y_encoded = np.asarray(classifier.y_encoder_.transform(y_true), dtype=np.int64)
    pred_indices = np.argmax(proba, axis=1)
    y_pred = np.asarray(classifier.classes_)[pred_indices]
    accuracy = float(np.mean(y_pred == y_true))

    clipped = np.clip(proba, 1e-12, 1.0)
    loss = float(-np.log(clipped[np.arange(y_encoded.shape[0]), y_encoded]).mean())
    return GateEvalResult(accuracy=accuracy, loss=loss)
...
def evaluate_one_dataset(classifier, dataset_dir: Path, ttt_config: TTTConfig | None = None) -> ResultRow:
    ...
    try:
        ...
        classes = pd.unique(pd.Series(np.concatenate([y_train_array, y_test_array], axis=0)))
        ...
        n_holdout_c = 0
        n_gate_d = 0
        gate_baseline_accuracy = None
        gate_adapted_accuracy = None
        gate_accuracy_delta = None
        gate_baseline_loss = None
        gate_adapted_loss = None
        gate_loss_delta = None

        t0 = time.time()
        if ttt_config.enabled:
            if should_skip_ttt_for_dataset(dataset_dir, info):
                ttt_gate_reason = "TTT skipped for dataset=volkert to avoid OOM"
                classifier.fit(X_train, y_train_array)
            else:
                split = split_ttt_gated_holdout(y_train_array, ttt_config)
                ...
                n_gate_d = int(len(split.d_indices))

                if n_train_b > 0 and n_holdout_c > 0 and n_gate_d > 0:
                    X_b = take_rows(X_train, split.b_indices)
                    y_b = y_train_array[split.b_indices]
                    X_c = take_rows(X_train, split.c_indices)
                    y_c = y_train_array[split.c_indices]
                    X_d = take_rows(X_train, split.d_indices)
                    y_d = y_train_array[split.d_indices]

                    label_reason = _label_subset_reason(y_b, y_c, y_d)
                    if label_reason is not None:
                        ttt_gate_reason = label_reason
                        classifier.fit(X_train, y_train_array)
                    else:
                        classifier.fit(X_b, y_b)
                        baseline_eval = evaluate_holdout_metrics(classifier, X_d, y_d)
                        gate_baseline_accuracy = baseline_eval.accuracy
                        gate_baseline_loss = baseline_eval.loss
                        ...
                        ttt_update_ran = bool(ttt_result.applied)
                        ttt_update_seconds = float(ttt_result.update_seconds)

                        if ttt_update_ran:
                            adapted_eval = evaluate_holdout_metrics(classifier, X_d, y_d)
                            gate_adapted_accuracy = adapted_eval.accuracy
                            gate_adapted_loss = adapted_eval.loss
                            gate_accuracy_delta = float(gate_adapted_accuracy - gate_baseline_accuracy)
                            gate_loss_delta = float(gate_adapted_loss - gate_baseline_loss)
                            ttt_gate_passed = bool(gate_accuracy_delta > ttt_config.gate_acc_threshold)
                            ttt_applied = ttt_gate_passed
                            comparator = ">" if ttt_gate_passed else "<="
                            ttt_gate_reason = (
                                f"Gate {'passed' if ttt_gate_passed else 'rejected'}: "
                                f"gate_accuracy_delta={gate_accuracy_delta:.6f} "
                                f"{comparator} threshold={ttt_config.gate_acc_threshold:.6f}"
                            )
                        else:
                            ttt_gate_reason = ttt_result.reason

                        if ttt_applied:
                            _fit_preserving_model_weights(classifier, X_train, y_train_array)
                        else:
                            classifier.fit(X_train, y_train_array)
                else:
                    ttt_gate_reason = "B/C/D split produced an empty side; skipped TTT"
                    classifier.fit(X_train, y_train_array)
        else:
            classifier.fit(X_train, y_train_array)
        ...
        accuracy = float(np.mean(np.asarray(y_pred) == y_test_array))
        ...
        return ResultRow(
            ...
            status="ok",
            error=None,
            n_train_a=int(len(y_train_array)),
            n_train_b=n_train_b,
            n_holdout_c=n_holdout_c,
            n_gate_d=n_gate_d,
            n_test_e=int(len(y_test_array)),
            ttt_loss=ttt_loss,
            ttt_steps=ttt_steps,
            ttt_lr=ttt_lr,
            ttt_update_ran=ttt_update_ran,
            ttt_gate_passed=ttt_gate_passed,
            ...
            gate_baseline_accuracy=gate_baseline_accuracy,
            gate_adapted_accuracy=gate_adapted_accuracy,
            gate_accuracy_delta=gate_accuracy_delta,
            gate_baseline_loss=gate_baseline_loss,
            gate_adapted_loss=gate_adapted_loss,
            gate_loss_delta=gate_loss_delta,
            ttt_gate_threshold=(ttt_config.gate_acc_threshold if ttt_config.enabled else None),
...
def write_summary(
    ...
    numeric_gate_acc_delta = (
        pd.to_numeric(ok_df["gate_accuracy_delta"], errors="coerce") if "gate_accuracy_delta" in ok_df else pd.Series(dtype=float)
    )
    numeric_gate_loss_delta = (
        pd.to_numeric(ok_df["gate_loss_delta"], errors="coerce") if "gate_loss_delta" in ok_df else pd.Series(dtype=float)
    )
    ttt_enabled_df = ok_df[ok_df["ttt_gate_threshold"].notna()].copy() if "ttt_gate_threshold" in ok_df else pd.DataFrame()

    gate_pass_count = int(ttt_enabled_df["ttt_gate_passed"].fillna(False).astype(bool).sum()) if len(ttt_enabled_df) else 0
    gate_reject_count = (
        int(
            (
                ttt_enabled_df["ttt_update_ran"].fillna(False).astype(bool)
                & ~ttt_enabled_df["ttt_gate_passed"].fillna(False).astype(bool)
            ).sum()
        )
        if len(ttt_enabled_df)
        else 0
    )
    ttt_skipped_count = int(len(ttt_enabled_df) - gate_pass_count - gate_reject_count)
    ...
    lines = [
        ...
        f"gate_pass_count: {gate_pass_count}",
        f"gate_reject_count: {gate_reject_count}",
        f"ttt_skipped_count: {ttt_skipped_count}",
        f"avg_gate_accuracy_delta_ok: {_format_optional_metric(numeric_gate_acc_delta.mean() if len(numeric_gate_acc_delta) else None)}",
        f"avg_gate_loss_delta_ok: {_format_optional_metric(numeric_gate_loss_delta.mean() if len(numeric_gate_loss_delta) else None)}",
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run TabICLv1.1 classification benchmarks with gated holdout TTT."
    )
    parser.add_argument("--data-root", default=str(DEFAULT_DATA_ROOT))
    parser.add_argument("--model-path", default=None)
    parser.add_argument("--models-dir", default=None)
    parser.add_argument("--checkpoint-version", default=DEFAULT_CHECKPOINT_VERSION)
    parser.add_argument("--out-dir", default="2a_gated_result/ttt_gated_step40_lr2e-6")
    ...
    parser.add_argument("--ttt-b-fraction", type=float, default=0.6)
    parser.add_argument("--ttt-c-fraction", type=float, default=0.3)
    parser.add_argument("--ttt-d-fraction", type=float, default=0.1)
    parser.add_argument("--ttt-gate-acc-threshold", type=float, default=0.0)
