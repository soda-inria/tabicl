...
class ResultRow:
    ...
    ttt_resume_ckpt: Optional[str] = None
    ttt_resume_step: Optional[int] = None
    ttt_resume_applied: bool = False
    ...
    *,
    ...
    if row.status == "ok":
        return (
            ...
            f"ttt_resume_applied={row.ttt_resume_applied} "
            f"ttt_resume_step={row.ttt_resume_step}"
        )
    if row.status == "skip":
...
def build_ttt_config(args: argparse.Namespace) -> TTTConfig:
    ...
    if int(args.ttt_resume_step) < 0:
        raise ValueError("--ttt-resume-step must be >= 0")
    ...
    return TTTConfig(
        ...
        resume_ckpt_root=str(Path(args.ttt_resume_ckpt_root).expanduser()),
        resume_step=int(args.ttt_resume_step),
        resume_model_name=(
            None
            if args.ttt_resume_model_name is None
            else (str(args.ttt_resume_model_name).strip() or None)
        ),
    )
...
def _resolve_ttt_resume_model_name(config: TTTConfig, model_name: str) -> str:
    return config.resume_model_name or model_name


def _build_ttt_resume_ckpt_path(config: TTTConfig, model_name: str, dataset_name: str) -> Path:
    resume_model_name = _resolve_ttt_resume_model_name(config, model_name)
    return (
        Path(config.resume_ckpt_root)
        / _sanitize_path_component(resume_model_name)
        / _sanitize_path_component(dataset_name)
        / f"step_{config.resume_step}.ckpt"
    )
...
def _load_ttt_resume_ckpt(
    ...
    *,
    ...
    if not isinstance(checkpoint, dict):
        raise ValueError(f"Resume checkpoint must be a dict: {ckpt_path}")
    if "config" not in checkpoint:
        raise ValueError(f"Resume checkpoint is missing config: {ckpt_path}")
    if "state_dict" not in checkpoint:
        raise ValueError(f"Resume checkpoint is missing state_dict: {ckpt_path}")

    metadata = checkpoint.get("ttt_metadata") or {}
    if isinstance(metadata, dict):
        metadata_step = metadata.get("step")
        if metadata_step is not None and int(metadata_step) != int(resume_step):
            raise ValueError(
                f"Resume checkpoint step mismatch for {dataset_name}: "
                f"metadata step={metadata_step}, expected step={resume_step}"
            )
        metadata_dataset = metadata.get("dataset_name")
        if metadata_dataset is not None and str(metadata_dataset) != str(dataset_name):
            raise ValueError(
                f"Resume checkpoint dataset mismatch: metadata dataset={metadata_dataset!r}, "
                f"expected dataset={dataset_name!r}"
            )
...
def run_ttt_holdout_update(
    ...
    *,
    ...
    print(
        f"[ttt-resume] loaded model={model_name} dataset={dataset_name} "
        f"step={config.resume_step} path={resume_ckpt_path}",
        flush=True,
    )
    if config.steps <= config.resume_step:
        return TTTUpdateResult(
            applied=True,
            loss=None,
            steps=config.steps,
            update_seconds=time.time() - update_start,
            reason=(
                f"Resume checkpoint step={config.resume_step} is already at or beyond "
                f"--ttt-steps={config.steps}; no optimizer steps run"
            ),
        )
    ...
    try:
        ...
        last_saved_step = 0
        for step_idx in range(config.resume_step + 1, config.steps + 1):
...
def evaluate_one_dataset(
    ...
    *,
    ...
    try:
        ttt_resume_ckpt = None
        ttt_resume_step = None
        ttt_resume_applied = False
        ...
        if ttt_config.enabled:
            resume_ckpt_path = _build_ttt_resume_ckpt_path(ttt_config, model_name, dataset_dir.name)
            ttt_resume_ckpt = resume_ckpt_path.as_posix()
            ttt_resume_step = int(ttt_config.resume_step)
            if not resume_ckpt_path.exists():
                return ResultRow(
                    ...
                    predict_seconds=0.0,
                    status="skip",
                    error=f"Missing resume ckpt: {resume_ckpt_path}",
                    ttt_lr=ttt_config.lr,
                    ttt_resume_ckpt=ttt_resume_ckpt,
                    ttt_resume_step=ttt_resume_step,
                    ttt_resume_applied=False,
                )
        ...
        if ttt_config.enabled:
            if should_skip_ttt_for_dataset(dataset_dir, info):
                ...
                ttt_resume_applied = bool(ttt_result.applied)
        ...
        return ResultRow(
            ...
            ttt_resume_ckpt=ttt_resume_ckpt,
            ttt_resume_step=ttt_resume_step,
            ttt_resume_applied=ttt_resume_applied,
        )
    except Exception as exc:
        return ResultRow(
            dataset_name=dataset_dir.name,
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Resume TabICLv1.1 ensemble TTT from dataset-matched checkpoints "
            "on data178 with AMD/ROCm multi-GPU workers."
        )
    )
    ...
    parser.add_argument(
        "--ttt-resume-model-name",
        default=None,
        help=(
            "Model directory name under --ttt-resume-ckpt-root. "
            "Defaults to the current derived model name."
        ),
    )
    parser.add_argument(
        "--ttt-data-parallel",
