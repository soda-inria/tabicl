...
from tabicl.train.run import ddp_cleanup, Timer
...
class TrainerCompFinetuner:
    """    Trainer for fine-tuning the TabICL model with the compression module.
    """
    ...
    def _freeze_predictor(self):
        ...
        for param in self.raw_model.col_embedder.parameters():
            param.requires_grad = False
        for param in self.raw_model.row_interactor.parameters():
            param.requires_grad = False
        self.raw_model.context_compression_transformer.train() 
