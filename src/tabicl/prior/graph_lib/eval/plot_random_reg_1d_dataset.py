...
def plot_random_dataset(cls):
    ...
    for row in range(nrows):
        for col in range(ncols):
            ...
            ds_prop = DatasetProperties(n_train=n_points, n_test=0, cat_sizes={"x": [0]*1, "y": [0]})
            while True:
    ...
    plt.tight_layout()
    plt.savefig(f"plots/{cls.__name__}_reg_1d.pdf", dpi=300)
    plt.show()
    plt.close()
