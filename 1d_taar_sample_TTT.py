...
class ResultRow:
    ...
    ttt_context_selector: Optional[str] = None
    ttt_taar_score_source: Optional[str] = None
    ttt_taar_selected_context_mean: Optional[float] = None
    ttt_taar_selected_context_min: Optional[int] = None
    ttt_taar_selected_context_max: Optional[int] = None
    ttt_taar_fallback_count: int = 0
    ttt_taar_skip_count: int = 0
    ttt_taar_feature_enabled: bool = False
    ttt_taar_feature_score_source: Optional[str] = None
    ttt_taar_selected_feature_mean: Optional[float] = None
    ttt_taar_selected_feature_min: Optional[int] = None
    ttt_taar_selected_feature_max: Optional[int] = None
    ttt_taar_feature_fallback_count: int = 0
...
class TTTConfig:
    ...
    context_selector: str = "random"
    taar_score_source: str = "auto"
    taar_feature_selection: bool = False
    taar_sample_selection: bool = False
    taar_feature_attn: float = 0.8
    taar_feature_ratio: float = 0.3
    taar_min_features: int = 1
    taar_max_features: Optional[int] = None
    taar_sample_attn: float = 0.8
    taar_context_ratio: float = 0.15
    taar_min_context: int = 300
    taar_max_context: Optional[int] = None
    taar_streaming_batch_size: int = 10_000
    taar_class_coverage_repair: bool = True
    taar_ablate_no_sample: bool = False
    taar_ablate_no_dynamic_threshold: bool = False
    taar_log_topk_overlap: bool = False
...
class TTTUpdateResult:
    ...
    taar_enabled: bool = False
    taar_score_source: Optional[str] = None
    taar_selected_context_mean: Optional[float] = None
    taar_selected_context_min: Optional[int] = None
    taar_selected_context_max: Optional[int] = None
    taar_fallback_count: int = 0
    taar_skip_count: int = 0
    taar_feature_enabled: bool = False
    taar_feature_score_source: Optional[str] = None
    taar_selected_feature_mean: Optional[float] = None
    taar_selected_feature_min: Optional[int] = None
    taar_selected_feature_max: Optional[int] = None
    taar_feature_fallback_count: int = 0
...
class MetaBatch:
    ...
    feature_selector_source: Optional[str] = None
    selected_feature_count: int = 0
    feature_selector_fallback: bool = False


@dataclass
class TAARSampleSelection:
    ctx_idx: Any
    qry_idx: Any
    context_scores: Optional[Any] = None
...
@dataclass
class TAARFeatureSelection:
    feature_idx: Any
    feature_scores: Optional[Any] = None
    score_source: str = "none"
    fallback_reason: Optional[str] = None
    selected_feature_count: int = 0
    used_fallback: bool = False
    ...
    *,
    ...
    if row.status == "ok":
        return (
            ...
            f"ttt_context_selector={row.ttt_context_selector} "
            f"ttt_taar_score_source={row.ttt_taar_score_source} "
            f"ttt_taar_ctx_mean={format_optional_float(row.ttt_taar_selected_context_mean)} "
            f"ttt_taar_fallback_count={row.ttt_taar_fallback_count} "
            f"ttt_taar_skip_count={row.ttt_taar_skip_count} "
            f"ttt_taar_feature_enabled={row.ttt_taar_feature_enabled} "
            f"ttt_taar_feature_source={row.ttt_taar_feature_score_source} "
            f"ttt_taar_feature_mean={format_optional_float(row.ttt_taar_selected_feature_mean)} "
            f"ttt_taar_feature_fallback_count={row.ttt_taar_feature_fallback_count}"
        )
    if row.status == "skip":
...
def cli_option_was_provided(option_name: str) -> bool:
...
def build_ttt_config(args: argparse.Namespace) -> TTTConfig:
    ...
    if str(args.ttt_context_selector) not in {"random", "taar_sample", "knn"}:
        raise ValueError("--ttt-context-selector must be one of: random, taar_sample, knn")
    if str(args.ttt_taar_score_source) not in {"auto", "attention", "gradient", "knn"}:
        raise ValueError("--ttt-taar-score-source must be one of: auto, attention, gradient, knn")
    if not 0.0 < float(args.ttt_taar_feature_attn) <= 1.0:
        raise ValueError("--ttt-taar-feature-attn must be in (0, 1]")
    if not 0.0 < float(args.ttt_taar_feature_ratio) <= 1.0:
        raise ValueError("--ttt-taar-feature-ratio must be in (0, 1]")
    if int(args.ttt_taar_min_features) < 1:
        raise ValueError("--ttt-taar-min-features must be >= 1")
    if args.ttt_taar_max_features is not None and int(args.ttt_taar_max_features) < int(args.ttt_taar_min_features):
        raise ValueError("--ttt-taar-max-features must be >= --ttt-taar-min-features or None")
    if not 0.0 < float(args.ttt_taar_sample_attn) <= 1.0:
        raise ValueError("--ttt-taar-sample-attn must be in (0, 1]")
    if not 0.0 < float(args.ttt_taar_context_ratio) <= 1.0:
        raise ValueError("--ttt-taar-context-ratio must be in (0, 1]")
    if int(args.ttt_taar_min_context) < 1:
        raise ValueError("--ttt-taar-min-context must be >= 1")
    if args.ttt_taar_max_context is not None and int(args.ttt_taar_max_context) < int(args.ttt_taar_min_context):
        raise ValueError("--ttt-taar-max-context must be >= --ttt-taar-min-context or None")
    if int(args.ttt_taar_streaming_batch_size) < 1:
        raise ValueError("--ttt-taar-streaming-batch-size must be >= 1")

    if cli_option_was_provided("--ttt-context-selector"):
        context_selector = str(args.ttt_context_selector)
    elif bool(args.ttt_taar_sample_selection):
        context_selector = "taar_sample"
    else:
        context_selector = "random"
    ...
    return TTTConfig(
        ...
        taar_score_source=str(args.ttt_taar_score_source),
        taar_feature_selection=bool(args.ttt_taar_feature_selection),
        taar_sample_selection=bool(args.ttt_taar_sample_selection),
        taar_feature_attn=float(args.ttt_taar_feature_attn),
        taar_feature_ratio=float(args.ttt_taar_feature_ratio),
        taar_min_features=int(args.ttt_taar_min_features),
        taar_max_features=(
            int(args.ttt_taar_max_features) if args.ttt_taar_max_features is not None else None
        ),
        taar_sample_attn=float(args.ttt_taar_sample_attn),
        taar_context_ratio=float(args.ttt_taar_context_ratio),
        taar_min_context=int(args.ttt_taar_min_context),
        taar_max_context=(
            int(args.ttt_taar_max_context) if args.ttt_taar_max_context is not None else None
        ),
        taar_streaming_batch_size=int(args.ttt_taar_streaming_batch_size),
        taar_class_coverage_repair=bool(args.ttt_taar_class_coverage_repair),
        taar_ablate_no_sample=bool(args.ttt_taar_ablate_no_sample),
        taar_ablate_no_dynamic_threshold=bool(args.ttt_taar_ablate_no_dynamic_threshold),
        taar_log_topk_overlap=bool(args.ttt_taar_log_topk_overlap),
...
def _make_ensemble_meta_batch(
    ...
    *,
    selector_source: Optional[str] = None,
    selected_context_count: Optional[int] = None,
    selector_fallback: bool = False,
) -> MetaBatch:
    try:
    ...
    y_ctx = np.asarray(y_chunk)[ctx_idx].astype(int)
    y_qry = np.asarray(y_chunk)[qry_idx].astype(int)
    ...
    return MetaBatch(
        ...
        selected_context_count=int(selected_context_count if selected_context_count is not None else train_size),
        selector_fallback=bool(selector_fallback),
    )


def _safe_normalize_scores(scores):
...
def _top_indices_by_cumulative_score(
    scores,
    coverage,
    min_count,
    max_count=None,
    proportion_floor=None,
):
    normalized = _safe_normalize_scores(scores)
    n_scores = int(normalized.size)
    if n_scores == 0:
        return np.asarray([], dtype=int)

    max_allowed = n_scores if max_count is None else min(int(max_count), n_scores)
    min_allowed = min(max(1, int(min_count)), n_scores)
    max_allowed = max(min_allowed, max_allowed)

    order = np.argsort(-normalized, kind="stable")
    cumulative = np.cumsum(normalized[order])
    k_att = int(np.searchsorted(cumulative, float(coverage), side="left") + 1)
    if proportion_floor is None:
        k_floor = 0
    else:
        k_floor = int(math.ceil(float(proportion_floor) * float(n_scores)))
    k = max(k_att, k_floor, min_allowed)
    k = min(max(k, min_allowed), max_allowed)
    return order[:k].astype(int, copy=False)


def _top_feature_indices_by_attention_budget(
    scores,
    coverage,
    proportion_cap,
    min_count,
    ...
    cumulative = np.cumsum(normalized[order])
    k_att = int(np.searchsorted(cumulative, float(coverage), side="left") + 1)
    k_prop = int(math.ceil(float(proportion_cap) * float(n_scores)))
    min_allowed = min(max(1, int(min_count)), n_scores)
    max_allowed = min(max(1, k_prop), n_scores)
    if max_count is not None:
        max_allowed = min(max_allowed, int(max_count), n_scores)
    max_allowed = max(min_allowed, max_allowed)
    k = min(k_att, max_allowed)
    k = min(max(k, min_allowed), n_scores)
    return order[:k].astype(int, copy=False)


def _sample_scoring_candidates(
    ctx_idx,
    y_chunk,
    config,
    rng,
    ...
    ctx_idx = np.asarray(ctx_idx, dtype=int)
    max_candidates = int(config.taar_streaming_batch_size)
    if len(ctx_idx) <= max_candidates:
        return ctx_idx

    positions = np.arange(len(ctx_idx))
    y_ctx = np.asarray(y_chunk)[ctx_idx]
    try:
        from sklearn.model_selection import train_test_split

        selected_pos, _ = train_test_split(
            positions,
            train_size=max_candidates,
            random_state=int(rng.integers(0, np.iinfo(np.int32).max)),
            shuffle=True,
            stratify=y_ctx,
        )
        selected_pos = np.asarray(selected_pos, dtype=int)
    except Exception:
        selected_pos = rng.choice(positions, size=max_candidates, replace=False)
    return ctx_idx[np.asarray(selected_pos, dtype=int)]
...
def _repair_class_coverage(
    ...
    selected_ctx_idx = np.asarray(selected_ctx_idx, dtype=int)
    if not config.taar_class_coverage_repair:
        return selected_ctx_idx

    candidate_ctx_idx = np.asarray(candidate_ctx_idx, dtype=int)
    qry_idx = np.asarray(qry_idx, dtype=int)
    y_arr = np.asarray(y_chunk)
    selected_labels = set(y_arr[selected_ctx_idx].astype(int).tolist()) if len(selected_ctx_idx) else set()
    query_labels = set(y_arr[qry_idx].astype(int).tolist()) if len(qry_idx) else set()
    missing_labels = sorted(query_labels - selected_labels)
    if not missing_labels:
        return selected_ctx_idx

    repaired = selected_ctx_idx.tolist()
    repaired_set = set(int(item) for item in repaired)
    scores = None if context_scores is None else np.asarray(context_scores, dtype=np.float64).reshape(-1)
    for label in missing_labels:
        label_positions = np.flatnonzero(y_arr[candidate_ctx_idx].astype(int) == int(label))
        if len(label_positions) == 0:
            continue
        available_positions = [
            int(pos)
            for pos in label_positions.tolist()
            if int(candidate_ctx_idx[int(pos)]) not in repaired_set
        ]
        if not available_positions:
            continue
        if scores is not None and len(scores) == len(candidate_ctx_idx):
            chosen_pos = max(available_positions, key=lambda pos: float(scores[pos]))
        else:
            chosen_pos = int(rng.choice(np.asarray(available_positions, dtype=int)))
        chosen_idx = int(candidate_ctx_idx[chosen_pos])
        repaired.append(chosen_idx)
        repaired_set.add(chosen_idx)

    return np.asarray(repaired, dtype=int)


def _make_pilot_meta_batch_no_taar(
    classifier,
    X_chunk,
    y_chunk,
    ctx_idx,
    ...
    return _make_ensemble_meta_batch(
        classifier,
        X_chunk,
        y_chunk,
        ctx_idx,
        qry_idx,
        config,
        selector_source="pilot",
        selected_context_count=int(len(ctx_idx)),
...
def _capture_training_modes(model) -> List[tuple[Any, bool]]:
    if hasattr(model, "modules"):
        return [(module, bool(module.training)) for module in model.modules()]
    return []


def _restore_training_modes(training_modes: List[tuple[Any, bool]]) -> None:
    for module, was_training in training_modes:
        module.train(was_training)


def _compute_sdpa_attention_weights(q, k, attn_mask=None, ssmax_layer=None):
    import torch

    q_used = q
    if ssmax_layer is not None:
        q_used = ssmax_layer(q_used, k.size(-2))
    scores = torch.matmul(q_used.float(), k.transpose(-2, -1).float())
    scores = scores / math.sqrt(float(max(1, q_used.shape[-1])))
...
@contextmanager
def capture_tabicl_attention_weights():
    import torch
    try:
        from tabicl._model import attention as attention_module
    except ModuleNotFoundError as exc:
        if exc.name not in {"tabicl._model", "tabicl._model.attention"}:
            raise
        from tabicl.model import attention as attention_module

    records: List[dict[str, Any]] = []
    errors: List[str] = []
    original_sdpa = attention_module.sdpa_with_flattened_batch

    def patched_sdpa(q, k, v, attn_mask=None, dropout_p=0.0, ssmax_layer=None):
        try:
            with torch.no_grad():
                weights = _compute_sdpa_attention_weights(
                    q.detach(),
                    k.detach(),
                    attn_mask.detach() if attn_mask is not None else None,
                ...
                records.append(
                    {
                        ...
                        "q_len": int(q.shape[-2]),
                        "src_len": int(k.shape[-2]),
                    }
                )
        except Exception as exc:
            errors.append(f"{type(exc).__name__}: {exc}")
        return original_sdpa(q, k, v, attn_mask, dropout_p, ssmax_layer=ssmax_layer)

    attention_module.sdpa_with_flattened_batch = patched_sdpa
    try:
        yield records, errors
    finally:
        attention_module.sdpa_with_flattened_batch = original_sdpa


def _extract_icl_sample_attention_scores(records, train_size: int, total_size: int):
    import torch

    candidates = [
        record
        for record in records
        if int(record.get("src_len", -1)) == int(train_size)
        and int(record.get("q_len", -1)) >= int(total_size)
    ]
    if not candidates:
        return None

    weights = candidates[-1]["weights"].float()
    query_to_context = weights[..., train_size:total_size, :train_size]
    if query_to_context.numel() == 0:
        return None
    scores = query_to_context.mean(dim=tuple(range(query_to_context.ndim - 1)))
    if int(scores.numel()) != int(train_size):
        return None
    scores = torch.nan_to_num(scores, nan=0.0, posinf=0.0, neginf=0.0)
    return scores.numpy()


def _extract_row_feature_attention_scores(records, train_size: int, total_size: int, n_features: int, num_cls: int):
    import torch

    expected_src_len = int(n_features) + int(num_cls)
    candidates = [
        record
        for record in records
        if int(record.get("q_len", -1)) == int(num_cls)
        and int(record.get("src_len", -1)) == expected_src_len
    ]
    if not candidates:
        return None
    ...
    weights = candidates[-1]["weights"].float()
    if weights.ndim >= 5:
        row_axis = weights.ndim - 4
        if int(weights.shape[row_axis]) >= int(total_size):
            index = [slice(None)] * weights.ndim
            index[row_axis] = slice(int(train_size), int(total_size))
            weights = weights[tuple(index)]

    feature_attn = weights[..., int(num_cls) : expected_src_len]
    if feature_attn.numel() == 0:
        return None
    scores = feature_attn.mean(dim=tuple(range(feature_attn.ndim - 1)))
    if int(scores.numel()) != int(n_features):
        return None
    scores = torch.nan_to_num(scores, nan=0.0, posinf=0.0, neginf=0.0)
    return scores.numpy()


def _compute_attention_sample_scores(
    classifier,
    forward_model,
    X_chunk,
    y_chunk,
    ...
    *,
    ...
    if forward_model is None:
        raise RuntimeError("forward_model is required for attention scoring")

    pilot = _make_pilot_meta_batch_no_taar(
        classifier,
        X_chunk,
        y_chunk,
        candidate_ctx_idx,
    ...
    train_size = int(pilot.train_size)
    total_size = int(pilot.X.shape[1])
    training_modes = _capture_training_modes(forward_model)
    try:
        forward_model.eval()
        X_pilot = pilot.X.to(device, non_blocking=True)
        y_train_pilot = pilot.y_train.to(device, non_blocking=True)
        with torch.inference_mode():
            with capture_tabicl_attention_weights() as (records, errors):
                _ = forward_model(X_pilot, y_train_pilot.float())
        scores = _extract_icl_sample_attention_scores(records, train_size, total_size)
    finally:
        _restore_training_modes(training_modes)
        del pilot

    if scores is None:
        detail = "; ".join(errors[-3:]) if errors else "no matching ICL attention map"
        raise RuntimeError(f"No usable sample attention tensor captured: {detail}")
    return _safe_normalize_scores(scores)


def _compute_attention_feature_scores(
    classifier,
    forward_model,
    X_chunk,
    y_chunk,
    ...
    *,
    ...
    if forward_model is None:
        raise RuntimeError("forward_model is required for feature attention scoring")
    ...
    train_size = int(pilot.train_size)
    total_size = int(pilot.X.shape[1])
    n_features = int(pilot.X.shape[2])
    base_model = _get_ttt_base_model(classifier)
    num_cls = int(getattr(getattr(base_model, "row_interactor", None), "num_cls", 4))
    training_modes = _capture_training_modes(forward_model)
    try:
        forward_model.eval()
        X_pilot = pilot.X.to(device, non_blocking=True)
        ...
        with torch.inference_mode():
            with capture_tabicl_attention_weights() as (records, errors):
                _ = forward_model(X_pilot, y_train_pilot.float())
        scores = _extract_row_feature_attention_scores(
            records,
            train_size=train_size,
    ...
    if scores is None:
        detail = "; ".join(errors[-3:]) if errors else "no matching row feature attention map"
        raise RuntimeError(f"No usable feature attention tensor captured: {detail}")
    return _safe_normalize_scores(scores)


def compute_taar_feature_selection_for_chunk(
    classifier,
    forward_model,
    X_chunk,
    y_chunk,
    ...
    *,
    ...
    epoch_idx: int,
    chunk_idx: int,
) -> TAARFeatureSelection:
    n_features = int(np.asarray(X_chunk).shape[1])
    full_feature_idx = np.arange(n_features, dtype=int)
    if not config.taar_feature_selection:
        return TAARFeatureSelection(
            feature_idx=full_feature_idx,
            feature_scores=None,
    ...
    try:
        feature_scores = _compute_attention_feature_scores(
            classifier,
            forward_model,
            X_chunk,
            y_chunk,
        ...
        if len(feature_scores) != n_features:
            raise RuntimeError(f"Feature score count mismatch: {len(feature_scores)} vs {n_features}")
        selected_feature_idx = _top_feature_indices_by_attention_budget(
            feature_scores,
            coverage=config.taar_feature_attn,
            proportion_cap=config.taar_feature_ratio,
            min_count=config.taar_min_features,
            max_count=config.taar_max_features,
        )
        selected_feature_idx = np.sort(np.asarray(selected_feature_idx, dtype=int), kind="stable")
        if selected_feature_idx.size == 0:
            raise RuntimeError("Feature selector produced no columns")
        print(
            f"[ttt-taar-feature] model={model_name} dataset={dataset_name} "
            f"epoch={epoch_idx + 1} chunk={chunk_idx} source=attention "
            f"selected={len(selected_feature_idx)}/{n_features}",
            flush=True,
        )
        return TAARFeatureSelection(
            ...
            selected_feature_count=int(len(selected_feature_idx)),
            used_fallback=False,
        )
    except Exception as exc:
        if is_oom_exception(exc):
            force_memory_cleanup(str(device))
        print(
            f"[ttt-taar-feature] model={model_name} dataset={dataset_name} "
            f"epoch={epoch_idx + 1} chunk={chunk_idx} source=fallback_all_features "
            f"selected={n_features}/{n_features} reason={type(exc).__name__}: {exc}",
            flush=True,
        )
        return TAARFeatureSelection(
            feature_idx=full_feature_idx,
            feature_scores=None,
            score_source="fallback_all_features",
...
def _compute_gradient_sample_scores(
    classifier,
    forward_model,
    X_chunk,
    y_chunk,
    ...
    *,
    ...
    if forward_model is None:
        raise RuntimeError("forward_model is required for gradient scoring")
    ...
    try:
        ...
        X_pilot = pilot.X.to(device, non_blocking=True).detach().requires_grad_(True)
        y_train_pilot = pilot.y_train.to(device, non_blocking=True)
        y_query_pilot = pilot.y_query.to(device, non_blocking=True)
        with torch.enable_grad():
            with amp_ctx_factory():
                logits = forward_model(X_pilot, y_train_pilot.float())
                n_classes = int(y_train_pilot.max().item()) + 1
                loss = F.cross_entropy(
                    logits[..., :n_classes].reshape(-1, n_classes),
                    y_query_pilot.long().reshape(-1),
                )
            loss.backward()

        if X_pilot.grad is None:
            raise RuntimeError("Input gradient is None")
        context_grad = X_pilot.grad.detach()[:, : int(pilot.train_size)]
        grad_view = context_grad.reshape(context_grad.shape[0], context_grad.shape[1], -1)
        scores = torch.linalg.vector_norm(grad_view.float(), ord=2, dim=2).mean(dim=0)
        if int(scores.numel()) != int(len(candidate_ctx_idx)):
            raise RuntimeError(
                f"Gradient score count mismatch: {int(scores.numel())} vs {int(len(candidate_ctx_idx))}"
            )
        return _safe_normalize_scores(scores.detach().cpu().numpy())
    finally:
        try:
            forward_model.zero_grad(set_to_none=True)
        except Exception:
...
def _compute_knn_sample_scores(
    X_chunk,
    candidate_ctx_idx,
    qry_idx,
):
    ...
    X_ctx = np.nan_to_num(X_arr[candidate_ctx_idx], nan=0.0, posinf=0.0, neginf=0.0)
    X_qry = np.nan_to_num(X_arr[qry_idx], nan=0.0, posinf=0.0, neginf=0.0)
    if X_ctx.ndim != 2 or X_qry.ndim != 2 or len(X_qry) == 0:
        raise RuntimeError("KNN scorer requires non-empty 2D context/query arrays")
    query_proto = X_qry.mean(axis=0)
    distances = np.linalg.norm(X_ctx - query_proto.reshape(1, -1), axis=1)
    return _safe_normalize_scores(1.0 / (distances + 1e-12))


def compute_taar_sample_scores_for_chunk(
    classifier,
    forward_model,
    X_chunk,
    y_chunk,
    ...
    *,
    ...
    ctx_idx = np.asarray(ctx_idx, dtype=int)
    qry_idx = np.asarray(qry_idx, dtype=int)
    rng = np.random.default_rng(int(config.random_state + epoch_idx * 104729 + chunk_idx * 7919))

    if config.taar_ablate_no_sample:
        return TAARSampleSelection(
            ctx_idx=ctx_idx,
            qry_idx=qry_idx,
            context_scores=None,
            score_source="ablate_no_sample",
            selected_context_count=int(len(ctx_idx)),
            used_fallback=False,
        )

    candidate_ctx_idx = _sample_scoring_candidates(ctx_idx, y_chunk, config, rng)
    if len(candidate_ctx_idx) == 0:
        raise RuntimeError("No candidate context samples available for selector")

    requested_source = str(config.taar_score_source)
    if requested_source == "auto":
        attempts = ["attention", "gradient", "knn"]
    else:
        attempts = [requested_source]

    context_scores = None
    score_source = "none"
    fallback_messages = []
    for attempt_idx, source in enumerate(attempts):
        try:
            if source == "attention":
                context_scores = _compute_attention_sample_scores(
                    classifier,
                    forward_model,
                    X_chunk,
                    y_chunk,
                ...
                context_scores = _compute_gradient_sample_scores(
                    classifier,
                    forward_model,
                    X_chunk,
                    y_chunk,
                ...
                context_scores = _compute_knn_sample_scores(X_chunk, candidate_ctx_idx, qry_idx)
            else:
                raise RuntimeError(f"Unsupported TAAR score source: {source}")
            score_source = source if attempt_idx == 0 else f"fallback_{source}"
            break
        except Exception as exc:
            fallback_messages.append(f"{source}:{type(exc).__name__}:{exc}")
            if requested_source != "auto":
                raise RuntimeError(
                    f"TAAR sample {source} scorer failed: {type(exc).__name__}: {exc}"
                ) from exc
            if is_oom_exception(exc):
                force_memory_cleanup(str(device))

    if context_scores is None:
        raise RuntimeError("All TAAR sample scorers failed: " + " | ".join(fallback_messages))

    context_scores = _safe_normalize_scores(context_scores)
    if len(context_scores) != len(candidate_ctx_idx):
        raise RuntimeError(
            f"TAAR sample score count mismatch: {len(context_scores)} vs {len(candidate_ctx_idx)}"
        )

    max_count = config.taar_max_context
    if config.taar_ablate_no_dynamic_threshold:
        n_candidates = int(len(candidate_ctx_idx))
        max_allowed = n_candidates if max_count is None else min(int(max_count), n_candidates)
        min_allowed = min(max(1, int(config.taar_min_context)), n_candidates)
        max_allowed = max(min_allowed, max_allowed)
        sample_k = int(math.ceil(float(config.taar_context_ratio) * float(n_candidates)))
        sample_k = min(max(sample_k, min_allowed), max_allowed)
        selected_positions = np.argsort(-context_scores, kind="stable")[:sample_k]
    else:
        selected_positions = _top_indices_by_cumulative_score(
            context_scores,
            coverage=config.taar_sample_attn,
            min_count=config.taar_min_context,
            max_count=max_count,
            proportion_floor=config.taar_context_ratio,
        )
    selected_ctx_idx = candidate_ctx_idx[np.asarray(selected_positions, dtype=int)]
    selected_ctx_idx = _repair_class_coverage(
        selected_ctx_idx,
        candidate_ctx_idx,
        qry_idx,
        y_chunk,
    ...
    if config.taar_log_topk_overlap:
        ratio_k = int(math.ceil(float(config.taar_context_ratio) * float(len(candidate_ctx_idx))))
        ratio_k = max(1, min(ratio_k, len(candidate_ctx_idx)))
        fixed_top = set(candidate_ctx_idx[np.argsort(-context_scores, kind="stable")[:ratio_k]].tolist())
        selected_set = set(np.asarray(selected_ctx_idx, dtype=int).tolist())
        overlap = len(selected_set & fixed_top) / float(max(1, len(selected_set)))
        print(
            f"[ttt-taar-sample-overlap] model={model_name} dataset={dataset_name} "
            f"epoch={epoch_idx + 1} chunk={chunk_idx} source={score_source} "
            f"selected={len(selected_set)} fixed_top={len(fixed_top)} overlap={overlap:.4f}",
            flush=True,
        )

    return TAARSampleSelection(
        ctx_idx=np.asarray(selected_ctx_idx, dtype=int),
        qry_idx=qry_idx,
        context_scores=context_scores,
        score_source=score_source,
        fallback_reason=" | ".join(fallback_messages) if fallback_messages else None,
        selected_context_count=int(len(selected_ctx_idx)),
        used_fallback=score_source.startswith("fallback_"),
    )
...
def _build_classification_meta_batch(
    ...
    *,
    ...
    selector_source = "random"
    selector_fallback = False
    selected_ctx_idx = np.asarray(ctx_idx, dtype=int)
    selected_context_count = int(len(selected_ctx_idx))
    feature_selection = compute_taar_feature_selection_for_chunk(
        classifier,
        forward_model,
        X_chunk,
        y_chunk,
    ...
    selected_feature_idx = np.asarray(feature_selection.feature_idx, dtype=int)
    X_selected_chunk = take_columns(X_chunk, selected_feature_idx)
    feature_selector_source = feature_selection.score_source
    feature_selector_fallback = bool(feature_selection.used_fallback)
    selected_feature_count = int(feature_selection.selected_feature_count)
    if config.context_selector == "taar_sample":
        try:
            selection = compute_taar_sample_scores_for_chunk(
                classifier,
                forward_model,
                X_selected_chunk,
                y_chunk,
            ...
            return MetaBatch(
                ...
                None,
                0,
                f"taar_sample selector failed after {split_strategy} split: {type(exc).__name__}: {exc}",
                selector_source="taar_sample",
                selected_context_count=0,
                selector_fallback=False,
                feature_selector_source=feature_selector_source,
                selected_feature_count=selected_feature_count,
                feature_selector_fallback=feature_selector_fallback,
            )
        selected_ctx_idx = np.asarray(selection.ctx_idx, dtype=int)
        selector_source = selection.score_source
        selector_fallback = bool(selection.used_fallback)
        selected_context_count = int(selection.selected_context_count)
    elif config.context_selector == "knn":
        knn_config = replace(config, taar_score_source="knn")
        try:
            selection = compute_taar_sample_scores_for_chunk(
                classifier,
                forward_model,
            ...
            return MetaBatch(
                ...
                None,
                0,
                f"knn selector failed after {split_strategy} split: {type(exc).__name__}: {exc}",
                selector_source="knn",
                selected_context_count=0,
                selector_fallback=False,
                feature_selector_source=feature_selector_source,
        ...
        selector_fallback = bool(selection.used_fallback)
        selected_context_count = int(selection.selected_context_count)
    elif config.context_selector != "random":
        return MetaBatch(
            None,
            None,
            None,
            0,
            f"unsupported context selector: {config.context_selector}",
            selector_source=str(config.context_selector),
            selected_context_count=0,
            selector_fallback=False,
            feature_selector_source=feature_selector_source,
            selected_feature_count=selected_feature_count,
    ...
    y_ctx_raw = np.asarray(y_chunk)[selected_ctx_idx].astype(int)
    ...
    if missing_query_labels:
        if config.context_selector == "random":
            reason = (
            ...
            reason = (
                f"{config.context_selector} missing query labels after selection: "
    ...
    batch = _make_ensemble_meta_batch(
        classifier,
        X_selected_chunk,
        y_chunk,
        selected_ctx_idx,
    ...
    batch.feature_selector_source = feature_selector_source
    batch.selected_feature_count = selected_feature_count
    batch.feature_selector_fallback = feature_selector_fallback
    return batch
...
def move_meta_batch(batch: MetaBatch, device) -> MetaBatch:
    return MetaBatch(
        ...
        selector_source=batch.selector_source,
        selected_context_count=batch.selected_context_count,
        selector_fallback=batch.selector_fallback,
        feature_selector_source=batch.feature_selector_source,
        selected_feature_count=batch.selected_feature_count,
        feature_selector_fallback=batch.feature_selector_fallback,
    )
...
def run_ttt_epoch_chunk_update(
    ...
    *,
    ...
    taar_enabled = config.context_selector in {"taar_sample", "knn"}
    taar_feature_enabled = bool(config.taar_feature_selection)
    selected_context_counts: List[int] = []
    selected_feature_counts: List[int] = []
    taar_fallback_count = 0
    taar_skip_count = 0
    taar_score_source_counts: Dict[str, int] = {}
    taar_feature_fallback_count = 0
    taar_feature_source_counts: Dict[str, int] = {}
    ...
    try:
        ...
        for epoch_idx in range(config.epochs):
            ...
            epoch_loss_sum = 0.0
            epoch_updates = 0
            epoch_selected_context_counts: List[int] = []
            epoch_selected_feature_counts: List[int] = []
            epoch_taar_fallback_count = 0
            epoch_taar_skip_count = 0
            epoch_taar_score_source_counts: Dict[str, int] = {}
            epoch_taar_feature_fallback_count = 0
            epoch_taar_feature_source_counts: Dict[str, int] = {}
            for chunk_idx, batch in enumerate(iter_epoch_meta_batches(
                classifier,
                X_encoded,
                y_encoded,
                config=config,
                ...
                if taar_enabled:
                    if batch.selected_context_count:
                        selected_context_counts.append(int(batch.selected_context_count))
                        epoch_selected_context_counts.append(int(batch.selected_context_count))
                    if batch.selector_source:
                        taar_score_source_counts[batch.selector_source] = (
                            taar_score_source_counts.get(batch.selector_source, 0) + 1
                        )
                        epoch_taar_score_source_counts[batch.selector_source] = (
                            epoch_taar_score_source_counts.get(batch.selector_source, 0) + 1
                        )
                    if batch.selector_fallback:
                        taar_fallback_count += 1
                        epoch_taar_fallback_count += 1
                if taar_feature_enabled:
                    if batch.selected_feature_count:
                        selected_feature_counts.append(int(batch.selected_feature_count))
                        epoch_selected_feature_counts.append(int(batch.selected_feature_count))
                    if batch.feature_selector_source:
                        taar_feature_source_counts[batch.feature_selector_source] = (
                            taar_feature_source_counts.get(batch.feature_selector_source, 0) + 1
                        )
                        epoch_taar_feature_source_counts[batch.feature_selector_source] = (
                            epoch_taar_feature_source_counts.get(batch.feature_selector_source, 0) + 1
                        )
                    if batch.feature_selector_fallback:
                        taar_feature_fallback_count += 1
                        epoch_taar_feature_fallback_count += 1
                if batch.skip_reason:
                    skipped_batches += 1
                    skip_reasons[batch.skip_reason] = skip_reasons.get(batch.skip_reason, 0) + 1
                    if taar_enabled:
                        taar_skip_count += 1
                        epoch_taar_skip_count += 1
                    print(
                        f"[ttt-skip] model={model_name} dataset={dataset_name} "
                        f"epoch={epoch_idx + 1}/{config.epochs} chunk={chunk_idx} "
                        f"selector={batch.selector_source} feature_source={batch.feature_selector_source} "
                        f"selected_ctx={batch.selected_context_count} "
                        f"selected_features={batch.selected_feature_count} "
                        f"reason={batch.skip_reason}",
                        flush=True,
                    )
                    continue
            ...
            if taar_enabled:
                if epoch_selected_context_counts:
                    epoch_mean_ctx = float(np.mean(epoch_selected_context_counts))
                    epoch_min_ctx = int(np.min(epoch_selected_context_counts))
                    epoch_max_ctx = int(np.max(epoch_selected_context_counts))
                else:
                    epoch_mean_ctx = float("nan")
                    epoch_min_ctx = 0
                    epoch_max_ctx = 0
                print(
                    f"[ttt-taar-sample] model={model_name} dataset={dataset_name} "
                    f"epoch={epoch_idx + 1}/{config.epochs} selector={config.context_selector} "
                    f"source_counts={dict(sorted(epoch_taar_score_source_counts.items()))} "
                    f"mean_ctx={epoch_mean_ctx:.2f} min_ctx={epoch_min_ctx} max_ctx={epoch_max_ctx} "
                    f"fallback={epoch_taar_fallback_count} skip={epoch_taar_skip_count}",
                    flush=True,
                )

            if taar_feature_enabled:
                if epoch_selected_feature_counts:
                    epoch_mean_feat = float(np.mean(epoch_selected_feature_counts))
                    epoch_min_feat = int(np.min(epoch_selected_feature_counts))
                    epoch_max_feat = int(np.max(epoch_selected_feature_counts))
                else:
                    epoch_mean_feat = float("nan")
                    epoch_min_feat = 0
                    epoch_max_feat = 0
                print(
                    f"[ttt-taar-feature] model={model_name} dataset={dataset_name} "
                    f"epoch={epoch_idx + 1}/{config.epochs} "
                    f"source_counts={dict(sorted(epoch_taar_feature_source_counts.items()))} "
                    f"mean_features={epoch_mean_feat:.2f} min_features={epoch_min_feat} "
                    f"max_features={epoch_max_feat} fallback={epoch_taar_feature_fallback_count}",
                    flush=True,
                )
        ...
        if update_steps == 0:
            ...
            dominant_source = (
                max(taar_score_source_counts.items(), key=lambda item: (item[1], item[0]))[0]
                if taar_score_source_counts
                else None
            )
            dominant_feature_source = (
                max(taar_feature_source_counts.items(), key=lambda item: (item[1], item[0]))[0]
                if taar_feature_source_counts
                else None
            )
            return TTTUpdateResult(
                applied=False,
                ...
                taar_enabled=taar_enabled,
                taar_score_source=dominant_source,
                taar_selected_context_mean=(
                    float(np.mean(selected_context_counts)) if selected_context_counts else None
                ),
                taar_selected_context_min=(
                    int(np.min(selected_context_counts)) if selected_context_counts else None
                ),
                taar_selected_context_max=(
                    int(np.max(selected_context_counts)) if selected_context_counts else None
                ),
                taar_fallback_count=taar_fallback_count,
                taar_skip_count=taar_skip_count,
                taar_feature_enabled=taar_feature_enabled,
                taar_feature_score_source=dominant_feature_source,
                taar_selected_feature_mean=(
                    float(np.mean(selected_feature_counts)) if selected_feature_counts else None
                ),
                taar_selected_feature_min=(
                    int(np.min(selected_feature_counts)) if selected_feature_counts else None
                ),
                taar_selected_feature_max=(
                    int(np.max(selected_feature_counts)) if selected_feature_counts else None
                ),
                taar_feature_fallback_count=taar_feature_fallback_count,
            )
...
def evaluate_one_dataset(
    ...
    *,
    ...
    try:
        ...
        ttt_context_selector = ttt_config.context_selector
        ttt_taar_score_source = None
        ttt_taar_selected_context_mean = None
        ttt_taar_selected_context_min = None
        ttt_taar_selected_context_max = None
        ttt_taar_fallback_count = 0
        ttt_taar_skip_count = 0
        ttt_taar_feature_enabled = bool(ttt_config.taar_feature_selection)
        ttt_taar_feature_score_source = None
        ttt_taar_selected_feature_mean = None
        ttt_taar_selected_feature_min = None
        ttt_taar_selected_feature_max = None
        ttt_taar_feature_fallback_count = 0
        n_train_b = 0
        n_holdout_c = 0
        ...
        if ttt_config.enabled:
            if should_skip_ttt_for_dataset(dataset_dir, info):
                ...
                try:
                    ...
                    ttt_taar_score_source = ttt_result.taar_score_source
                    ttt_taar_selected_context_mean = ttt_result.taar_selected_context_mean
                    ttt_taar_selected_context_min = ttt_result.taar_selected_context_min
                    ttt_taar_selected_context_max = ttt_result.taar_selected_context_max
                    ttt_taar_fallback_count = ttt_result.taar_fallback_count
                    ttt_taar_skip_count = ttt_result.taar_skip_count
                    ttt_taar_feature_enabled = ttt_result.taar_feature_enabled
                    ttt_taar_feature_score_source = ttt_result.taar_feature_score_source
                    ttt_taar_selected_feature_mean = ttt_result.taar_selected_feature_mean
                    ttt_taar_selected_feature_min = ttt_result.taar_selected_feature_min
                    ttt_taar_selected_feature_max = ttt_result.taar_selected_feature_max
                    ttt_taar_feature_fallback_count = ttt_result.taar_feature_fallback_count
        ...
        return ResultRow(
            ...
            ttt_context_selector=ttt_context_selector,
            ttt_taar_score_source=ttt_taar_score_source,
            ttt_taar_selected_context_mean=ttt_taar_selected_context_mean,
            ttt_taar_selected_context_min=ttt_taar_selected_context_min,
            ttt_taar_selected_context_max=ttt_taar_selected_context_max,
            ttt_taar_fallback_count=ttt_taar_fallback_count,
            ttt_taar_skip_count=ttt_taar_skip_count,
            ttt_taar_feature_enabled=ttt_taar_feature_enabled,
            ttt_taar_feature_score_source=ttt_taar_feature_score_source,
            ttt_taar_selected_feature_mean=ttt_taar_selected_feature_mean,
            ttt_taar_selected_feature_min=ttt_taar_selected_feature_min,
            ttt_taar_selected_feature_max=ttt_taar_selected_feature_max,
            ttt_taar_feature_fallback_count=ttt_taar_feature_fallback_count,
        )
    except Exception as exc:
        return ResultRow(
            dataset_name=dataset_dir.name,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--out-dir", default="1b_result/1d_taar_sample_ttt_chunk8000")
    parser.add_argument("--workers", type=int, default=1)
    parser.add_argument("--gpus", default=None)
    parser.add_argument("--gpu-groups", default="2")
    ...
    parser.add_argument("--ttt-max-chunk-size", type=int, default=8_000)
    ...
    parser.add_argument(
        "--ttt-context-selector",
        choices=["random", "taar_sample", "knn"],
        default="taar_sample",
    )
    parser.add_argument(
        "--ttt-taar-score-source",
        choices=["auto", "attention", "gradient", "knn"],
        default="attention",
    )
    parser.add_argument("--ttt-taar-feature-selection", type=parse_bool, default=False)
    parser.add_argument("--ttt-taar-sample-selection", type=parse_bool, default=True)
    parser.add_argument("--ttt-taar-feature-attn", type=float, default=0.8)
    parser.add_argument("--ttt-taar-feature-ratio", type=float, default=0.3)
    parser.add_argument("--ttt-taar-min-features", type=int, default=1)
    parser.add_argument("--ttt-taar-max-features", type=parse_optional_int, default=None)
    parser.add_argument("--ttt-taar-sample-attn", type=float, default=0.8)
    parser.add_argument("--ttt-taar-context-ratio", type=float, default=0.3)
    parser.add_argument("--ttt-taar-min-context", type=int, default=300)
    parser.add_argument("--ttt-taar-max-context", type=parse_optional_int, default=None)
    parser.add_argument("--ttt-taar-streaming-batch-size", type=int, default=8_000)
    parser.add_argument("--ttt-taar-class-coverage-repair", type=parse_bool, default=True)
    parser.add_argument("--ttt-taar-ablate-no-sample", type=parse_bool, default=False)
    parser.add_argument("--ttt-taar-ablate-no-dynamic-threshold", type=parse_bool, default=False)
    parser.add_argument("--ttt-taar-log-topk-overlap", type=parse_bool, default=False)
    parser.add_argument(
        "--ttt-save-ckpt",
        type=parse_bool,
        default=False,
