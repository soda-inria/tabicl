...
def build_ttt_config(args: argparse.Namespace) -> TTTConfig:
    ...
    return TTTConfig(
        ...
        lora_train_icl_head=bool(args.ttt_lora_train_icl_head),
    )
...
def _lora_attn_forward(
    ...
    try:
        from tabicl.model.attention import multi_head_attention_forward as attention_forward
    except ImportError:
        from tabicl.model.attention import multi_head_attention_forward as attention_forward
...
def _install_lora_on_attention(attn, rank: int, alpha: float, dropout: float) -> None:
    ...
    if not getattr(attn, "_ttt_lora_in_proj_installed", False):
        ...
        attn.add_module(_lora_param_name("in_proj_dropout"), nn.Dropout(p=float(dropout)))
...
def _install_ttt_lora(model, config: TTTConfig) -> None:
    blocks = getattr(getattr(model.icl_predictor, "tf_icl", None), "blocks", None)
    if blocks is None:
        raise RuntimeError("LoRA requested but model.icl_predictor.tf_icl.blocks was not found")
...
def _configure_ttt_trainable_params(classifier, config: TTTConfig):
    ...
    if not config.lora and not config.freeze_col:
        _set_requires_grad(model.col_embedder, True)

    model.row_interactor.train()
    if not config.lora and not config.freeze_row:
    ...
    if config.lora:
        ...
        if config.lora_train_icl_head:
            if hasattr(model.icl_predictor, "y_encoder"):
                _set_requires_grad(model.icl_predictor.y_encoder, True)
            if hasattr(model.icl_predictor, "ln"):
                _set_requires_grad(model.icl_predictor.ln, True)
            if hasattr(model.icl_predictor, "decoder"):
                _set_requires_grad(model.icl_predictor.decoder, True)
    else:
...
def _save_ttt_model_ckpt(classifier, config: TTTConfig, model_name: str, dataset_name: str, step_idx: int) -> Path:
    ...
    checkpoint = {
        ...
        "state_dict": _merged_ttt_state_dict(base_model),
        "ttt_metadata": {
            ...
            "lora_train_icl_head": bool(config.lora_train_icl_head),
            "lora_merged_into_state_dict": True,
        },
    }
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--out-dir", default="1b_result/lora/lora_ensem_step8_lr5e-6")
    ...
    parser.add_argument(
        ...
        type=parse_bool,
        default=True,
        help="Use script-local LoRA adapters for TTT instead of full ICL predictor training.",
    )
    ...
    parser.add_argument(
        "--ttt-lora-train-icl-head",
        type=parse_bool,
        default=True,
        help="When LoRA is enabled, also train icl_predictor y_encoder/ln/decoder.",
    )
    parser.add_argument(
        "--ttt-save-ckpt",
        type=parse_bool,
