...
class ResultRow:
    ...
    ttt_lora_enabled: bool = False
    ttt_lora_targets: Optional[str] = None
    ttt_lora_rank: int = 0
    ttt_lora_trainable_params: int = 0
    ...
    *,
    ...
    if row.status == "ok":
        return (
            ...
            f"ttt_lora_enabled={row.ttt_lora_enabled} "
            f"ttt_lora_targets={row.ttt_lora_targets} "
            f"ttt_lora_trainable_params={row.ttt_lora_trainable_params}"
        )
    if row.status == "skip":
...
def parse_lora_targets(value: str) -> str:
    raw_targets = [item.strip().lower() for item in str(value).split(",")]
    targets = []
    for target in raw_targets:
        if not target:
            continue
        if target not in {"row", "icl"}:
            raise argparse.ArgumentTypeError("must be a comma-separated subset of: row, icl")
        if target not in targets:
            targets.append(target)
    if not targets:
        raise argparse.ArgumentTypeError("must include at least one of: row, icl")
    return ",".join(targets)
...
def build_ttt_config(args: argparse.Namespace) -> TTTConfig:
    ...
    lora_targets = parse_lora_targets(args.ttt_lora_targets)
...
def _lora_param_name(name: str) -> str:
    return f"_ttt_lora_{name}"


def _lora_target_set(config: TTTConfig) -> set[str]:
    return set(parse_lora_targets(config.lora_targets).split(","))


def _lora_linear_delta(module):
    import torch

    lora_a = getattr(module, _lora_param_name("A"), None)
    lora_b = getattr(module, _lora_param_name("B"), None)
    if lora_a is None or lora_b is None:
        return torch.zeros_like(module.weight)
    return (lora_b @ lora_a) * float(getattr(module, "_ttt_lora_scaling", 1.0))


def _lora_in_proj_delta(attn):
    import torch

    lora_a = getattr(attn, _lora_param_name("in_proj_A"), None)
    lora_b = getattr(attn, _lora_param_name("in_proj_B"), None)
    if lora_a is None or lora_b is None:
        return torch.zeros_like(attn.in_proj_weight)
    return (lora_b @ lora_a) * float(getattr(attn, "_ttt_lora_in_proj_scaling", 1.0))


def _lora_linear_forward(module, input):
    import torch.nn.functional as F

    if not getattr(module, "_ttt_lora_installed", False):
        original_forward = getattr(module.__class__, "_ttt_lora_original_forward", None)
        if original_forward is not None:
            return original_forward(module, input)
        return F.linear(input, module.weight, module.bias)

    base = F.linear(input, module.weight, module.bias)
    dropout = getattr(module, _lora_param_name("dropout"), None)
    lora_input = dropout(input) if dropout is not None else input
    return base + F.linear(lora_input, _lora_linear_delta(module), None)


def _get_lora_attention_forward():
    try:
...
def _get_lora_attention_class():
    try:
        ...
        if exc.name not in {"tabicl._model", "tabicl._model.layers"}:
            raise
        from tabicl.model.layers import MultiheadAttention

    return MultiheadAttention


def _lora_attn_forward(
    self,
    query,
    key=None,
    value=None,
    ...
    if not getattr(self, "_ttt_lora_forward_installed", False):
        original_forward = getattr(self.__class__, "_ttt_lora_original_forward", None)
        if original_forward is None:
            raise RuntimeError("LoRA attention forward patch is missing the original forward")
    ...
    attention_forward = _get_lora_attention_forward()
    if key is None:
        key = query
    if value is None:
        value = key
    ...
    return attention_forward(
        query,
        self.num_heads,
        self.in_proj_weight + _lora_in_proj_delta(self),
        self.in_proj_bias,
        self.dropout,
        self.out_proj.weight + _lora_linear_delta(self.out_proj),
        self.out_proj.bias,
        key=key,
        value=value,
        cached_kv=cached_kv,
...
def _patch_lora_forward_methods() -> None:
    from torch import nn

    if not getattr(nn.Linear, "_ttt_lora_class_forward_patched", False):
        nn.Linear._ttt_lora_original_forward = nn.Linear.forward
        nn.Linear.forward = _lora_linear_forward
        nn.Linear._ttt_lora_class_forward_patched = True

    MultiheadAttention = _get_lora_attention_class()
    if not getattr(MultiheadAttention, "_ttt_lora_class_forward_patched", False):
        MultiheadAttention._ttt_lora_original_forward = MultiheadAttention.forward
        MultiheadAttention.forward = _lora_attn_forward
        MultiheadAttention._ttt_lora_class_forward_patched = True


def _install_lora_on_linear(module, rank: int, alpha: float, dropout: float) -> None:
    _patch_lora_forward_methods()
    if getattr(module, "_ttt_lora_installed", False):
        return
    ...
    lora_a = nn.Parameter(torch.empty(rank, weight.shape[1], device=weight.device, dtype=weight.dtype))
    lora_b = nn.Parameter(torch.zeros(weight.shape[0], rank, device=weight.device, dtype=weight.dtype))
    nn.init.kaiming_uniform_(lora_a, a=5**0.5)
    module.register_parameter(_lora_param_name("A"), lora_a)
    module.register_parameter(_lora_param_name("B"), lora_b)
    module.add_module(_lora_param_name("dropout"), nn.Dropout(p=float(dropout)))
    module._ttt_lora_scaling = float(alpha) / float(rank)
    module._ttt_lora_installed = True


def _install_lora_on_attention(attn, rank: int, alpha: float, dropout: float) -> None:
    _patch_lora_forward_methods()
    if not getattr(attn, "_ttt_lora_in_proj_installed", False):
        import torch
        from torch import nn
        ...
        lora_b = nn.Parameter(torch.zeros(weight.shape[0], rank, device=weight.device, dtype=weight.dtype))
        nn.init.kaiming_uniform_(lora_a, a=5**0.5)
        attn.register_parameter(_lora_param_name("in_proj_A"), lora_a)
        attn.register_parameter(_lora_param_name("in_proj_B"), lora_b)
        attn._ttt_lora_in_proj_scaling = float(alpha) / float(rank)
        attn._ttt_lora_in_proj_installed = True

    _install_lora_on_linear(attn.out_proj, rank, alpha, dropout)
    attn._ttt_lora_forward_installed = True


def _get_transformer_blocks(module, attr_path: str):
    current = module
    for attr_name in attr_path.split("."):
        current = getattr(current, attr_name)
    blocks = getattr(current, "blocks")
    if not blocks:
        raise RuntimeError(f"LoRA target {attr_path}.blocks was not found")
    return blocks


def _install_lora_on_blocks(blocks, config: TTTConfig) -> None:
    for block in blocks:
        _install_lora_on_attention(block.attn, config.lora_rank, config.lora_alpha, config.lora_dropout)
        _install_lora_on_linear(block.linear1, config.lora_rank, config.lora_alpha, config.lora_dropout)
        _install_lora_on_linear(block.linear2, config.lora_rank, config.lora_alpha, config.lora_dropout)


def _install_ttt_lora(model, config: TTTConfig) -> None:
    targets = _lora_target_set(config)
    if "row" in targets:
        _install_lora_on_blocks(_get_transformer_blocks(model, "row_interactor.tf_row"), config)
    if "icl" in targets:
        _install_lora_on_blocks(_get_transformer_blocks(model, "icl_predictor.tf_icl"), config)


def _set_lora_requires_grad(model, requires_grad: bool = True) -> None:
    for name, param in model.named_parameters():
        if "._ttt_lora_" in name or name.startswith("_ttt_lora_"):
            param.requires_grad = requires_grad


def _count_trainable_params(params) -> int:
    return int(sum(param.numel() for param in params if param.requires_grad))


def _merged_ttt_state_dict(model):
    state_dict = {}
    modules = dict(model.named_modules())
    raw_state_dict = model.state_dict()

    for name, tensor in raw_state_dict.items():
        if "._ttt_lora_" in name or name.rsplit(".", 1)[-1].startswith("_ttt_lora_"):
            continue

        merged = tensor.detach()
        if name.endswith(".in_proj_weight"):
            module_name = name[: -len(".in_proj_weight")]
            module = modules.get(module_name)
            if module is not None and getattr(module, "_ttt_lora_in_proj_installed", False):
                merged = merged + _lora_in_proj_delta(module).detach()
        elif name.endswith(".weight"):
            module_name = name[: -len(".weight")]
            module = modules.get(module_name)
            if module is not None and getattr(module, "_ttt_lora_installed", False):
                merged = merged + _lora_linear_delta(module).detach()

        state_dict[name] = merged.cpu()
...
def _configure_ttt_trainable_params(classifier, config: TTTConfig):
    ...
    if config.lora:
        ...
        _install_ttt_lora(model, config)
...
def _save_ttt_model_ckpt(classifier, config: TTTConfig, model_name: str, dataset_name: str, step_idx: int) -> Path:
    ...
    checkpoint = {
        ...
        "state_dict": (
            _merged_ttt_state_dict(base_model)
            if config.lora
            else {name: tensor.detach().cpu() for name, tensor in base_model.state_dict().items()}
        ),
        "ttt_metadata": {
            ...
            "lora_targets": str(config.lora_targets) if config.lora else None,
            "lora_rank": int(config.lora_rank) if config.lora else 0,
            "lora_alpha": float(config.lora_alpha) if config.lora else 0.0,
            "lora_dropout": float(config.lora_dropout) if config.lora else 0.0,
            "lora_merged_into_state_dict": bool(config.lora),
        },
    }
...
def run_ttt_epoch_chunk_update(
    ...
    *,
    ...
    lora_trainable_params = _count_trainable_params(trainable_params) if config.lora else 0
...
def evaluate_one_dataset(
    ...
    *,
    ...
    try:
        ...
        ttt_lora_enabled = bool(ttt_config.enabled and ttt_config.lora)
        ttt_lora_targets = str(ttt_config.lora_targets) if ttt_lora_enabled else None
        ttt_lora_rank = int(ttt_config.lora_rank) if ttt_lora_enabled else 0
        ttt_lora_trainable_params = 0
        n_train_b = 0
        n_holdout_c = 0
        ...
        if ttt_config.enabled:
            if should_skip_ttt_for_dataset(dataset_dir, info):
                ...
                try:
                    ...
                    ttt_lora_enabled = ttt_result.lora_enabled
                    ttt_lora_targets = ttt_result.lora_targets
                    ttt_lora_rank = ttt_result.lora_rank
                    ttt_lora_trainable_params = ttt_result.lora_trainable_params
        ...
        return ResultRow(
            ...
            ttt_lora_enabled=ttt_lora_enabled,
            ttt_lora_targets=ttt_lora_targets,
            ttt_lora_rank=ttt_lora_rank,
            ttt_lora_trainable_params=ttt_lora_trainable_params,
        )
    except Exception as exc:
        return ResultRow(
            dataset_name=dataset_dir.name,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=parse_bool,
        default=True,
        help="Use row_interactor and icl_predictor LoRA adapters for the TTT update path.",
    )
    parser.add_argument(
        "--ttt-lora-targets",
        type=parse_lora_targets,
        default="row,icl",
        help="Comma-separated LoRA target modules. Supported values: row, icl.",
    )
    parser.add_argument("--ttt-lora-rank", type=int, default=8)
    parser.add_argument("--ttt-lora-alpha", type=float, default=16.0)
