...
def run_pseudo_label_resplit_inference(
    *,
    ...
    2. 用真实标签监督交叉熵 ce=-log(q_true) 选出 ce <= ce_alpha 的样本加入伪标签集合。
    ...
    """
    if not np.isfinite(float(ce_alpha)) or float(ce_alpha) < 0.0:
        raise ValueError(f"{dataset_name}: ce_alpha 必须是有限且 >= 0 的数值，当前为 {ce_alpha}")
    ...
    if classes is None:
        raise ValueError(f"{dataset_name}: 分类器缺少 classes_，无法计算监督交叉熵")
    classes = np.asarray(classes)
    baseline_pred = classes[pred_indices]
    baseline_accuracy = float(np.mean(np.asarray(baseline_pred) == y_test))

    true_class_indices = pd.Index(classes).get_indexer(pd.Index(y_test))
    if (true_class_indices < 0).any():
        missing_labels = pd.Index(y_test[true_class_indices < 0]).unique().tolist()
        raise ValueError(
            f"{dataset_name}: y_test 中存在未出现在 classifier.classes_ 里的标签: {missing_labels}"
        )

    true_probs = probs[np.arange(probs.shape[0]), true_class_indices]
    pseudo_ce = -np.log(np.clip(true_probs.astype(float), PSEUDO_CE_EPS, 1.0))
    confident_mask = np.asarray(pseudo_ce <= float(ce_alpha))
    ...
    result: dict[str, Any] = {
        ...
        "pseudo_ce_alpha": float(ce_alpha),
...
def evaluate_datasets_worker(
    ...
    *,
    ...
    pseudo_label_ce_alpha: float | None = None,
    random_state: int = 42,
    ...
    try:
        ...
        for dataset_dir in assigned_datasets:
            ...
            try:
                ...
                if pseudo_label_ce_alpha is not None:
                    pseudo_result = run_pseudo_label_resplit_inference(
                        ...
                        y_test=y_test,
                        dataset_name=dataset_dir.name,
                        ce_alpha=pseudo_label_ce_alpha,
                    )
                ...
                results.append(
                    {
                        ...
                        "pseudo_ce_alpha": pseudo_label_ce_alpha,
                        "train_ratio_before": train_ratio_before,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--outdir", default="tabiclv1.1_pesudo_test_ce0.15", help="结果输出目录")
    ...
    parser.add_argument(
        ...
        help=(
            "启用二阶段伪标签重划分。"
            "第一轮按真实标签监督交叉熵 ce=-log(q_true) 筛选 ce <= ce_alpha 的样本打伪标签，"
...
def run_single_shard(args: argparse.Namespace, classifier_config: dict[str, Any]) -> int:
    ...
    results, datasets_with_missing = evaluate_datasets_worker(
        ...
        pseudo_label_ce_alpha=args.pseudo_label_ce_alpha,
        random_state=args.random_state,
    )
...
def write_results(
    ...
    *,
    ...
    pseudo_enabled = any(item.get("pseudo_ce_alpha") is not None for item in results)
    baseline_values = [
    ...
    pseudo_ce_alpha_value = next(
        (item.get("pseudo_ce_alpha") for item in results if item.get("pseudo_ce_alpha") is not None),
        None,
    )
    ...
    with csv_path.open("a", encoding="utf-8") as handle:
        if write_header:
            handle.write(
                ...
                "pseudo_added,pseudo_applied,pseudo_ce_alpha,train_ratio_before,train_ratio_after\n"
            )
        for item in results:
            baseline_accuracy = item.get("baseline_accuracy")
            pseudo_ce_alpha = item.get("pseudo_ce_alpha")
            handle.write(
                "{dataset},{accuracy:.6f},{time_s:.3f},{fit_time_s:.3f},{predict_time_s:.3f},{baseline_accuracy},{pseudo_added},{pseudo_applied},{pseudo_ce_alpha},{train_ratio_before:.6f},{train_ratio_after:.6f}\n".format(
                    ...
                    pseudo_ce_alpha=("" if pseudo_ce_alpha is None else f"{float(pseudo_ce_alpha):.6f}"),
    ...
    with summary_path.open("a", encoding="utf-8") as handle:
        ...
        if pseudo_enabled:
            handle.write(f"Pseudo label CE alpha: {pseudo_ce_alpha_value}\n")
...
def main(argv=None) -> int:
    ...
    if args.pseudo_label_ce_alpha is not None and (
        not np.isfinite(args.pseudo_label_ce_alpha) or args.pseudo_label_ce_alpha < 0.0
    ):
        parser.error("--pseudo-label-ce-alpha 必须是有限且 >= 0 的数值")
